...
import { parseFeatureFlags } from "@iopn/shared";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { VoteButtons } from "@/components/tokens/vote-buttons";
import { shortenAddress, formatTaxBps } from "@/lib/utils";
...
export default function TokenPage() {
  ...
  const features = parseFeatureFlags(Number(token.featureFlags));
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2">
        {token.buyTaxBps != null && (
          <Card>
            <CardContent className="pt-6">
              <p className="text-sm text-muted-foreground">Buy tax</p>
              <p className="text-xl font-semibold">{formatTaxBps(token.buyTaxBps)}</p>
            </CardContent>
          </Card>
        )}
        {token.sellTaxBps != null && (
          <Card>
            <CardContent className="pt-6">
              <p className="text-sm text-muted-foreground">Sell tax</p>
              <p className="text-xl font-semibold">{formatTaxBps(token.sellTaxBps)}</p>
            </CardContent>
          </Card>
        )}
      </div>
